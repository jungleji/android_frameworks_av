#ifndef _RK_BLURAY_TEXTSUBTITLE_H_
#define _RK_BLURAY_TEXTSUBTITLE_H_
...
public:
    ...
    {
//        mTextLength = 0;
        mText = (unsigned char*)new char[TEXTST_MAX_CHARS_PER_DPS_REGION];
        mInLineStyleValues = (unsigned char*)new char[TEXTST_MAX_CHARS_PER_DPS_REGION];
        mInLineStyleType = (unsigned char*)new char[TEXTST_MAX_CHARS_PER_DPS_REGION];
        mInLineStylePosition = (unsigned char*)new char[TEXTST_MAX_CHARS_PER_DPS_REGION];

        if(mText != NULL)
        {
            memset(mText,0,TEXTST_MAX_CHARS_PER_DPS_REGION);
        }
    }
    ...
    ~TextLineData()
    {
        ...
        if(mInLineStyleValues != NULL)
        {
            delete[] mInLineStyleValues;
            mInLineStyleValues = NULL;
        }

        if(mInLineStyleType != NULL)
        {
            delete[] mInLineStyleType;
            mInLineStyleType = NULL;
        }

        if(mInLineStylePosition != NULL)
        {
            delete[] mInLineStylePosition;
            mInLineStylePosition = NULL;
        }
    }
public:
//    int                  mTextLength;
...
public:
    ...
    Vector<TextLineData*> *mLineData;
public:
    DialogRegion()
    {
        mContinousPresentFlag = 0;
        mForceOnFlag = 0;
        mRegionStyleRefId = 0;
        

        mLineData = new Vector<TextLineData*>();
    }
    ...
    ~DialogRegion()
    {
        if(mLineData != NULL)
        {
            ...
            {
                TextLineData* data = mLineData->itemAt(i);
                if(data != NULL)
                {
                    delete data;
                    data = NULL;
