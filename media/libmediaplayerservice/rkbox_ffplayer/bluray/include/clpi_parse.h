...
uint32_t clpi_lookup_spn(const CLPI_CL *cl, uint64_t timestamp, int before, uint8_t stc_id);
uint32_t clpi_access_point(const CLPI_CL *cl, uint32_t pkt, int next, int angle_change, uint32_t *time);
CLPI_CL* clpi_parse(const char *path);
