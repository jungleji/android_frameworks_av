...
status_t FFPlayer::setDataSource(const char *url, const KeyedVector<String8, String8> *headers) {
...
#ifdef AVS44
status_t FFPlayer::setVideoSurfaceTexture(const sp<IGraphicBufferProducer> &bufferProducer) {
    ALOGV("setVideoSurfaceTexture");
    return mPlayer->setSurfaceTexture(bufferProducer);
}
#else
status_t FFPlayer::setVideoSurfaceTexture(const sp<ISurfaceTexture> &surfaceTexture) {
    ALOGV("setVideoSurfaceTexture");
...
status_t FFPlayer::getMetadata(const media::Metadata::Filter& ids, Parcel *records) {
    ALOGV("getMetadata");
